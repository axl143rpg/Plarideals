...
$memberId = $_GET['id'];
$selectQuery = "SELECT consumers.*, name.type AS Consumer_name_name
                FROM consumers
                JOIN name ON consumers.Consumer_name = name.id
                WHERE id = $id";
...
if ($result->num_rows > 0) {
    ...
    $consumershipStatus = ($daysSinceCreation > 30) ? 'Expired' : 'Active';
} else {
    header("Location: consumers_list.php");
    exit();
}
...
<head>
    ...
    <title>Print consumership Card</title>
    <style>
        body {
            background: #222;
            padding: 2rem;
...
<body>
    <div class="card">
        <span class="title">consumership card
        ...
        <span class="emboss"><b>#<?php echo $memberDetails['consumership_number']; ?></b></span>
        ...
        <div>
            <span class="emboss"><b>consumership: <?php echo $memberDetails['Consumer_name_name']; ?></b></span>
